...
msg = '''Given a BASENAME (-b) (e.g. "DUBLIN_"), an absolute PATH (-p) and baseline REFERENCE (-r) for CROCO time (e.g. "19680523"), this 
         function renames CROCO history files to a more human-readable format for archiving, such as DUBLIN_20250530.nc '''
...
def main(): 
    ...
    files = sorted(glob.glob(path + '/croco_*.nc'))

    for f in files:
        with Dataset(f, 'r') as nc: # Open NetCDF
            time = num2date(nc.variables['time'][:], 'seconds since ' + units.strftime('%Y-%m-%d')) # Read time 
        # Rename file using date from first time stamp
        name = path + '/' + basename + time[0].strftime('%Y%m%d') + '.nc'; print(os.path.basename(name))
